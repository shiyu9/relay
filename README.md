...
- **Stop** — 未記録の transcript が残ったまま進んでいるときだけ、記録を促す。プロジェクト単位で30分のクールダウンがあり、公式の `stop_hook_active` ガードで自分の促しには反応しない
  - 促しは**その時点で残っているものだけを名指しし、手順そのものを載せる。**起動時のジョブは「起動時点で未記録だったもの」を指しているので、それを消化したあとで同じ場所を指すと、記録済みセッションの再実行になってしまう
  - **渡したばかりのジョブについては促さない**（`jobs/<id>.md` が15分以内なら実行中とみなす）。記録サブエージェントは終了を通知しないので、並行実行中のジョブを「未記録」と数えて促す事故があった
